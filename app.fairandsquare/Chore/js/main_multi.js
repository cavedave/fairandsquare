...
//                return false;
            ...
            $('#btn_addTennant').click(function(){
              if ($('#input_tennant').val()==''){ //add here that only 2 names can be entered
              }
              else{
            ...
            $('#btn_addRoom').click(function(){
              if ($('#input_roomname').val()==''){
                ...
                $('#tab-rooms .output ul').append("<li>"+$('#input_roomname').val()+"   "+ "<input type='button' id='x' value='x' onclick='removeA("+a+",divisItem)'>"+"</li>");
...
function glpkTest(data){
    ...
    var peopleContr = addPeopleConstr();
    model = model + peopleContr; 
    model=model+data; console.log("testing testing 12345" + model);
    console.log("testing peopleContr" + model);
...
var lp = glp_create_prob();
     ...
     if(lp) {
        ...
        $('#outputresults').text("Percentage of work each person has to do is: "); 
        console.log("testing 123");
        $('#outputresults').append(glp_mip_obj_val(lp).toFixed(2).toString());
 // Create text with jQuery
        $('#outputresults').append("<p></p>")
        $('#outputresults').append("The FairAndSquare task allocation is: ");
        $('#outputresults').append("<p></p>");
...
function addPeopleConstr() {
//s.t. wor2{i in I}: ((sum{k in K} divItems[2,k] * x[2,k])+(sum{j in J} indivItems[2,j] * y[2,j])) >= worst;
//maximize obj: worst;
//this function creates the people constraint for every people
// argument is number of names taken in
//s.t. wor2{i in I}: ((sum{k in K} divItems[2,k] * x[2,k])+(sum{j in J} indivItems[2,j] * y[2,j])) >= worst;
  //we want to add a constraint for each person more than 2
  var constraints="";
  constraints= constraints +  "s.t. wor{i in I}: ((sum{k in K} divItems[1,k] * x[1,k])+(sum{j in J} indivItems[1,j] * y[1,j])) <= worst;\n"
  for (var i = 2; i<=people.length; i++ ) {
    constraints= constraints + "s.t. wor"+i+"{i in I}: ((sum{k in K} divItems["+i+",k] * x["+i+",k])+(sum{j in J} indivItems["+i+",j] * y["+i+",j])) <= worst;\n";
  }
    constraints= constraints + "minimize obj: worst;";
    return constraints;
  }
