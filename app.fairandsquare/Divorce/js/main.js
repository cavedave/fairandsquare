              ...
              if (people.length == 0 || indivItem.length+divisItem.length == 0) {
                ...
                if (stage > people.length) {
                  ...
                  $('#tab-preferences .output').html("          <h2>"+people[stage]+"</h2>\n          <h3>Divide 100 points between all "+numItem+" items</h3>");
                  sum=0;
                  allocation=100;
...
arr.splice(pos, 1)    
      if(arr==people){
        ...
        for(i=0;i<arr.length;i++){
          $('#tab-poeple .output ul').append("<li>"+arr[i]+ "<input type='button' id='x' value='x' onclick='removeA("+i+",people)'>"+"</li>");  
        }                
      }
      else if(arr==indivItem){
        $('#tab-rooms .output ul').text("");  
        for(i=0;i<divisItem.length;i++){
          $('#tab-rooms .output ul').append("<li>"+divisItem[i]+ "<input type='button' id='x' value='x' onclick='removeA("+i+",divisItem)'>"+"</li>");                
        }
        for(i=0;i<arr.length;i++){
          $('#tab-rooms .output ul').append("<li>"+arr[i]+ "<input type='button' id='x' value='x' onclick='removeA("+i+",indivItem)'>"+"</li>");                
        }
      }
      else if(arr==divisItem){
        $('#tab-rooms .output ul').text("");
        for(i=0;i<arr.length;i++){
          $('#tab-rooms .output ul').append("<li>"+arr[i]+ "<input type='button' id='x' value='x' onclick='removeA("+i+",divisItem)'>"+"</li>");                
        }
        for(i=0;i<indivItem.length;i++){
          $('#tab-rooms .output ul').append("<li>"+indivItem[i]+ "<input type='button' id='x' value='x' onclick='removeA("+i+",indivItem)'>"+"</li>");                
        }
      }
}
